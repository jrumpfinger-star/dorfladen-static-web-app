...
**Status:** Umgesetzt und ausgerollt. Offen: Bestandsübernahme ausführen
(T113/T114), Doku nachziehen (T115), Lesebrücke später zurückbauen (T119/T120)
und die Freigabe für die echten Empfängeradressen (T901).
...
- [x] **T001** Vor dem Start auf `origin/main` rebasen. Eine parallele Sitzung
...
- [x] **T002** `fpdf2` und `pypdf` in `api/requirements.txt` ergänzen.
      Beides reines Python, keine Systembibliotheken. — `F20`, `F22`

## Stammdaten (unabhängig vom Code)

- [x] **T010** `[P]` Startkatalog `api/baecker-order/vorlage/katalog-martins.json`
...
- [x] **T011** `[P]` `vorlage/katalog.json` → `vorlage/katalog-freundl.json`
...
- [x] **T020** `BAECKEREIEN = ("freundl", "martins")`, `ALT_BAECKEREI`,
...
- [x] **T021** **Lesebrücke** in `load_order` und `load_artikel`: fehlt der neue
      ...
      / `TC-B2-F24-02`
- [x] **T022** `store.bestellungen(url, hdrs, bk)` als **einzige** Stelle, die
...
- [x] **T025** `vorlage_bestellungen` und `nummer_umziehen` auf
...
- [x] **T026** `tools/baecker_store_test.py` — Lesebrücke und Schlüssel-Zuordnung
...
- [x] **T030** `baeckerei` aus Query **und** Rumpf lesen, gegen `BAECKEREIEN`
...
- [x] **T031** `_build_entwurf` und `_senden` auf Bäckerei umstellen
      (Katalog, Vorbelegung, Kd.-Nr., Tour-Nr., Korrektursperre). — `F17`, `F18`
- [x] **T032** `_uebersicht` liefert je Tag eine **Liste** `lieferanten` mit
...
- [x] **T032b** **Erinnerungsblock** in `_uebersicht` (~Z. 200–224) über **alle**
...
- [x] **T035** `aktion: 'gedruckt'` — setzt `gedruckt_am` an der Bestellung.
      Nur zulässig, wenn die Bestellung gesendet ist. — `F23` / `TC-B2-F23-06`
- [x] **T036** `_config_pruefen` auf die neue Struktur erweitern: je Bäckerei
...
- [x] **T040** Zeichenbereiniger `_latin1(text)`: bekannte Sonderzeichen
...
- [x] **T041** Formular bauen: Kopf (Bäckereiname, Anschrift), Zeile mit
...
- [x] **T042** Testbetrieb-Kennzeichnung im Dokument, sobald der Empfänger von
      der hinterlegten Bäckerei-Adresse abweicht — wie bei Freundl. — `F20`
- [x] **T043** `tools/baecker_pdf_test.py` — Formular erzeugen, mit `pypdf`
...
- [x] **T050** `rechnung_parser.py` — PDF-Text mit `pypdf` lesen, Positionen
      erkennen: Artikelnummer (steht am **Zeilenende**), Bezeichnung, Liefer-
      und Retourmenge. — `F22` / `TC-B2-F22-01`
- [x] **T051** Mehrzeilige Positionen zusammenführen. **Konkreter Fall:**
...
- [x] **T052** `tools/baecker_rechnung_test.py` — gegen alle **11** vorliegenden
...
- [x] **T060** `__init__.py`: `baeckerei` in GET/POST/PATCH; Dublettenprüfung je
      Bäckerei (Nr. 1 darf bei beiden existieren). — `F17`, `F21`
      / `TC-B2-F17-01`, `TC-B2-F21-04`
- [x] **T061** `aktion: 'rechnung'`, Modus **Vorschau** — liefert `neu`,
...
- [x] **T062** `aktion: 'rechnung'`, Modus **Übernahme** — schreibt
...
- [x] **T070** `function.json` + Grundgerüst mit `admin_auth_guard`.
      Parameter `?modus=test|echt`. — `F24`
- [x] **T071** **Testmodus**: zählt Bestellungen, Positionen und Artikel vorher
      und nachher, meldet Abweichungen, schreibt **nichts**. — `F24`
      / `TC-B2-F24-04`
- [x] **T072** **Echtmodus**: kopiert Katalog und alle Bestellungen auf die
...
- [x] **T073** `tools/baecker_migration_test.py` — zwei Teile:
...
- [x] **T080** `_baeckerei` in den Modulzustand; alle API-Aufrufe geben sie mit.
      Tageswechsel schaltet auf die dort liefernde Bäckerei um. — `F18`
      / `TC-B2-F18-02`
- [x] **T081** Tagesleiste: farbige Punkte je Lieferant, gesendete erkennbar,
      „1 von 2" an Tagen mit zwei Bäckereien. — `F18`, `F19`
      / `TC-B2-F18-03`, `TC-B2-F19-02`
- [x] **T082** Reiterzeile `.bk-btabs` — **nur** bei zwei Lieferanten, mit
      Abzeichen `offen` / `✓ gesendet` / `🖨 Ausdruck fehlt`. — `F19`
      / `TC-B2-F19-01`, `TC-B2-F19-03`
- [x] **T083** Statuskarte und Fußzeile nennen die Bäckerei im Klartext, auch an
      Ein-Bäckerei-Tagen. — `F18` / `TC-B2-F18-01`
- [x] **T084** Druckansicht: HTML aus den Positionsdaten bauen,
      `window.open('', '_blank')` + `window.print()` — dasselbe Muster wie
      `K.printKitchen()`. Mit Kd.-Nr., Tour-Nr., Liefertag und Retourenspalte.
      — `F23` / `TC-B2-F23-01`
- [x] **T085** Bestätigung nach dem Senden: Overlay mit der Schrittfolge
...
- [x] **T085b** Ausdruck-Zustand in der Oberfläche: Tagesplättchen
...
- [x] **T085c** Tab-Zähler: offene Bestellungen **und** offene Ausdrucke zählen.
      — `F19` / `TC-B2-F19-05`
- [x] **T086** Nachdruck aus dem Verlauf: die Bestellung per
      `GET ?baeckerei=…&datum=…` **nachladen** — `_verlauf` liefert nur die
      Anzahl der Positionen, nicht die Zeilen. Kein erneuter Mailversand.
      — `F23` / `TC-B2-F23-04`
- [x] **T087** Rechnungs-Import, Teil 1: Datei wählen, hochladen, **Vorschau**
      mit `neu` / `geändert` / `unverändert`; freundliche Meldung bei
      unlesbarem PDF. — `F22` / `TC-B2-F22-05`
- [x] **T087b** Rechnungs-Import, Teil 2: **Übernehmen** und Retouren-Übersicht
      mit Quote. — `F22` / `TC-B2-F22-06`
- [x] **T088** CSS in `kiosk.html`: `.bk-btabs`, Tagespunkte, Zustand
...
- [x] **T090** Bäckerei-Auswahl oben in der Karte „Bäckerei-Bestellung";
      alle Felder darunter gehören zur gewählten. Neue Felder: Anzeigename,
      Formularformat, Papierausdruck ja/nein. — `F25` / `TC-B2-F25-01`
- [x] **T091** Laden und Speichern je Bäckerei; Speichern darf die **andere**
      nicht anfassen. — `F25` / `TC-B2-F25-02`
- [x] **T092** Testbetrieb-Hinweis je Bäckerei; Hinweis (kein Fehler) bei
      überschneidenden Bestelltagen. — `F25` / `TC-B2-F25-03`, `TC-B2-F25-04`

## Tests

- [x] **T100** `tests/kiosk-baecker.spec.js` — Mock um `baeckerei` erweitern.
...
- [x] **T101** `tests/kiosk-baecker-zwei.spec.js` anlegen. Der Mock ist **eine**
...
- [x] **T102** Tests F21–F23 (Katalog, Rechnungs-Oberfläche, Ausdruck).
      — `TC-B2-F21-01…04`, `F22-05`, `-06`, `F23-01…06`
- [x] **T103** Tests F25–F26 (CMS, Responsive auf allen drei Auflösungen).
      — `TC-B2-F25-01…04`, `F26-01…04`
- [x] **T104** API-Tests über die `request`-Fixture: gleiche Nummer bei beiden
...
- [x] **T110** Volle Suite lokal, alle drei Auflösungen. Umgebungsvariable heißt
      **`TEST_URL`**, nicht `BASE_URL` — mit falschem Namen laufen die Tests
      still gegen die Standard-URL und bestehen irreführend.
- [x] **T111** Auf die **Testumgebung** ausrollen, Suite dort fahren.
- [x] **T112** Live ausrollen. Dank der Lesebrücke arbeitet der Tab mit dem
...
- [x] **T900** ~~Bestelladresse von Martin's klären~~ — **erledigt (07.09.2026):**
      Die Adresse wird je Bäckerei **im CMS gepflegt** (F25). In der Testphase
      steht sie bei **beiden** Bäckereien auf `jrumpfinger@t-online.de`.
      Die echte Bestelladresse von Martin's trägt der Nutzer selbst ein, sobald
      sie feststeht — `rechnung@martins-backstube.de` ist keine.
- [ ] **T901** Freigabe, den Empfänger von der Testadresse auf die echten
      Bäckereien umzustellen — für **beide** getrennt. Erfolgt im CMS, nicht im
      Code.
