...
(function(){
  ...
  function mobUnlockScroll(){
    var top=document.body.style.top;document.body.style.overflow='';document.body.style.position='';document.body.style.width='';document.body.style.top='';
  ...
  window.mobOpenPopup=function(id){
    ...
    if(el){
      ...
      if(window.pushPopupState)window.pushPopupState();
    }
  };
  ...
  function renderWP(menu){
    ...
    for(var i=1;i<=5;i++){
      ...
      // Bestellschluss: heute bis konfigurierte Uhrzeit (Fallback 10:30),
      // zukünftige Tage erlaubt, vergangene nicht.
  ...
  // Filter angebote by current week (So-Sa, matching desktop filterByWeek)
  function filterMobByWeek(items){
    var now=new Date();
    var wStart=new Date(now);wStart.setDate(wStart.getDate()-wStart.getDay());wStart.setHours(0,0,0,0);
    var wEnd=new Date(wStart);wEnd.setDate(wEnd.getDate()+6);wEnd.setHours(23,59,59);
  ...
  function loadMobPreisliste(){
    ...
    fetch(API_BASE+'/preisliste').then(function(r){return r.json();}).then(function(data){
      ...
      var rows=grid.querySelectorAll('.mob-pl-row:not(.mob-pl-header)');
      ...
      function applyPlFilters(){
        ...
        grid.querySelectorAll('.mob-pl-group').forEach(function(g){
          ...
          g.querySelectorAll('.mob-pl-row:not(.mob-pl-header)').forEach(function(r){if(r.style.display!=='none'){hasVisible=true;visCount++;}});
