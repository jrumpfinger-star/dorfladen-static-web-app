...
(function(){
  ...
  function socialShareFiles(files,msg,hasMt){var canShareFiles=false;if(navigator.share&&navigator.canShare){try{canShareFiles=navigator.canShare({files:files});}catch(e){}}if(canShareFiles){var shareData={files:files.length>1?[files[0]]:files};if(msg&&!socialIsMobile())shareData.text=msg;navigator.share(shareData).then(function(){socialStatus('soc-post-status','\u2705 Poster geteilt!',true);if(msg&&socialIsMobile()){try{navigator.clipboard.writeText(msg);}catch(e){}setTimeout(function(){window.open('https://wa.me/?text='+encodeURIComponent(msg),'_blank');socialStatus('soc-post-status','\u2705 Bestelllinks werden gesendet...',true);},800);}}).catch(function(err){if(err.name==='AbortError')return;socialShareViaClipboard(files,msg);});socialStatus('soc-post-status','Poster wird geteilt...',true);return;}if(socialIsMobile()){socialShareViaClipboard(files,msg);return;}if(msg){try{navigator.clipboard.writeText(msg);}catch(e){}socialStatus('soc-post-status','\uD83D\uDCCB Bestelltext kopiert!',true);}socialFallbackDownloadFiles(files);}
