...
(function(){
  ...
  function socialInitBildHandlers(){
    ...
    if(zone){
      zone.addEventListener('dragover',function(e){ e.preventDefault(); zone.style.borderColor='#e1306c'; zone.style.background='#fef2f2'; });
      zone.addEventListener('dragleave',function(){ if(!_socPastedFile&&!(inp&&inp.files&&inp.files.length)){ zone.style.borderColor='#d1d5db'; zone.style.background='#fafbfc'; } });
  ...
  function socialRenderKatalog(){
    ...
    Object.keys(cats).forEach(function(cat){
      ...
      html+='<div class="cms-card soc-card" style="margin-bottom:10px">';
      html+='<div class="cms-card-header soc-kat-cat-hdr" onclick="socialKatToggleCat(\''+catId+'\')" style="background:#1f2937;cursor:pointer;display:flex;align-items:center;justify-content:space-between;user-select:none;padding:10px 14px;color:#fff;font-weight:700;border-radius:10px 10px 0 0">';
      ...
      cats[cat].forEach(function(p,i){
        ...
        if(p.bild_url) html+='<img id="soc-kat-thumb-'+pid+'" src="'+esc(p.bild_url)+'" style="width:44px;height:44px;object-fit:cover;border-radius:6px;border:1px solid #e5e7eb" onerror="this.style.display=\'none\'">';
        else html+='<div id="soc-kat-thumb-'+pid+'" style="width:44px;height:44px;background:#f3f4f6;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:20px;color:#9ca3af">&#128247;</div>';
        ...
        html+='<label title="Bild \u00e4ndern" style="padding:4px 8px;font-size:14px;margin-right:3px;cursor:pointer;display:inline-flex;align-items:center;background:#f3f4f6;border:1px solid #d1d5db;border-radius:6px">&#128247;<input type="file" accept="image/*" capture="environment" onchange="socialKatImgChange(\''+pid+'\',this)" style="display:none"></label>';
        html+='<button onclick="socialKatEdit(\''+pid+'\')" title="Bearbeiten" style="padding:4px 8px;font-size:14px;margin-right:3px;background:#f3f4f6;border:1px solid #d1d5db;border-radius:6px;cursor:pointer">&#9998;</button>';
        html+='<button onclick="socialKatDelete(\''+pid+'\')" title="L\u00f6schen" style="color:#dc2626;padding:4px 8px;font-size:14px;background:#f3f4f6;border:1px solid #d1d5db;border-radius:6px;cursor:pointer">&#10005;</button>';
        html+='</td></tr>';
        html+='<tr id="soc-edit-'+pid+'" style="display:none;background:#fffbeb;border-bottom:2px solid #f59e0b"><td colspan="4" style="padding:10px">';
  ...
  function socialBuildPostItems(){ var wrap=document.getElementById('soc-post-items'); if(!wrap)return; var todayMeals=socialGetTodayMeals(); var allCats=[]; _socialKatalog.forEach(function(p){var c=p.kategorie||'Sonstiges';if(allCats.indexOf(c)===-1)allCats.push(c);}); var html='';
    ...
    if(todayMeals.length){ var days=['Sonntag','Montag','Dienstag','Mittwoch','Donnerstag','Freitag','Samstag']; var today=days[new Date().getDay()]; html+='<div style="margin-bottom:10px;background:#fff8e1;border:1px solid #ffe082;border-radius:10px;padding:8px 12px"><div style="font-size:12px;font-weight:700;color:#f57f17;margin-bottom:5px">&#127869; Heutiges Mittagessen ('+esc(today)+')</div>';
      todayMeals.forEach(function(m){ var wpId='wp-'+m.id; var mtImg=_socMtBilder[m.gericht]; html+='<div class="soc-mt-row" style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:#fff;border:2px solid #ffe082;border-radius:10px;margin-bottom:4px;min-height:44px"><label style="display:flex;align-items:center;gap:6px;flex:1;cursor:pointer"><input type="checkbox" class="soc-post-wp" value="'+esc(wpId)+'" data-name="'+esc(m.gericht)+'" data-preis="'+esc(m.preis?m.preis.toFixed(2):'')+'" data-kat="Mittagessen" data-img="'+esc(mtImg&&mtImg.bild_url?mtImg.bild_url:'')+'" onchange="socialPickUpdate()" style="width:18px;height:18px;accent-color:#f57f17">';
        if(mtImg&&mtImg.bild_url) html+='<img src="'+esc(mtImg.bild_url)+'" style="width:32px;height:32px;object-fit:cover;border-radius:4px;flex-shrink:0" onerror="this.style.display=\'none\'">';
    ...
    if(_socialKatalog.length){ html+='<div style="margin-bottom:8px"><input id="soc-pick-search" type="text" placeholder="&#128269; Produkt suchen..." oninput="socialPickFilter()" style="width:100%;padding:8px 12px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;outline:none;box-sizing:border-box"></div>';
      ...
      _socialKatalog.forEach(function(p){ var pid=esc(p.id); var priceStr=''; if(p.preis){var cp=parseFloat(p.preis);priceStr=(cp&&isFinite(cp)?cp.toFixed(2):esc(p.preis))+'\u20AC';}
        ...
        if(p.bild_url) html+='<img id="soc-pick-img-'+pid+'" src="'+esc(p.bild_url)+'" onclick="socialPickImgPreview(\''+pid+'\')" style="width:40px;height:40px;object-fit:cover;border-radius:6px;border:1px solid #e5e7eb;display:block;cursor:zoom-in" onerror="this.style.display=\'none\'">';
        else html+='<div id="soc-pick-img-'+pid+'" style="width:40px;height:40px;background:#f3f4f6;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:18px;color:#9ca3af;border:1px solid #e5e7eb">&#128247;</div>';
