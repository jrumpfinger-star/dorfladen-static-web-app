...
(function(){
  ...
  function socPopulateKatSelects() {
    var opts = socKatOptionsHtml('');
    ['soc-kat-kategorie'].forEach(function(id) {
      var el = document.getElementById(id);
      if (el) {
        var cur = el.value;
        // Neutrale Pflicht-Vorauswahl: nichts faellt automatisch auf "Mittagessen"
        // (verhindert versehentlich bestellbare Artikel bei uebernommenem Default).
        el.innerHTML = '<option value="" disabled' + (cur ? '' : ' selected') + '>\u2014 Kategorie w\u00e4hlen \u2014</option>' + opts;
        if (cur) el.value = cur;
      }
    });
  }
  ...
  function socialRenderKatalog(){
    ...
    Object.keys(cats).forEach(function(cat){
      ...
      html+='<div class="soc-kat-cat-hdr" onclick="socialKatToggleCat(\''+catId+'\')" style="background:linear-gradient(135deg,var(--c-pri,#2d5016) 0%,var(--c-green,#2e7d4f) 100%);cursor:pointer;display:flex;align-items:center;justify-content:space-between;user-select:none;padding:10px 14px;color:#fff;font-weight:600;border-radius:10px 10px 0 0">';
      ...
      cats[cat].forEach(function(p,i){
        ...
        if(p.bild_url) html+='<img id="soc-kat-thumb-'+pid+'" src="'+esc(p.thumb_url||p.bild_url)+'" data-full="'+esc(p.bild_url)+'" loading="lazy" decoding="async" ondblclick="dlImagePopup(this.getAttribute(\'data-full\')||this.src,\''+esc(p.name).replace(/'/g,"\\'")+'\')" style="width:44px;height:44px;object-fit:cover;border-radius:6px;border:1px solid #e5e7eb;cursor:zoom-in" onerror="this.style.display=\'none\'">';
        ...
        html+='</td><td style="padding:8px"><span style="font-weight:600">'+esc(p.name)+'</span></td>';
        html+='<td style="padding:8px;text-align:right;white-space:nowrap">';
        if(p.preis){var lp=parseFloat(p.preis);html+='<span style="font-weight:600;color:#2e7d32">'+(lp&&isFinite(lp)?lp.toFixed(2).replace('.',','):esc(p.preis))+' &#8364;</span>';}
  ...
  window.socialKatMgrRemove=function(idx){
    var catName=_socKategorien[idx].name;
    dlConfirm({icon:'🗑️',title:'Kategorie entfernen?',msg:'«'+catName+'» wird entfernt.',ok:'Entfernen',color:'#dc2626'},function(){
  ...
  function socialBuildPostItems(){ var wrap=document.getElementById('soc-post-items'); if(!wrap)return; var _prevChecked={}; wrap.querySelectorAll('input.soc-post-wp:checked, input.soc-post-cb:checked').forEach(function(cb){_prevChecked[cb.value]=true;}); var todayMeals=socialGetTodayMeals(); _socialKatalog.sort(function(a,b){return (a.name||'').localeCompare(b.name||'','de');}); var allCats=[]; _socialKatalog.forEach(function(p){var c=p.kategorie||'Sonstiges';if(allCats.indexOf(c)===-1)allCats.push(c);}); var html='';
    ...
    // Free entry
    html+='<div style="margin-bottom:10px"><button onclick="socialFreeToggle()" style="width:100%;padding:12px 14px;background:#fff;border:1px dashed #cfd4cd;border-radius:10px;cursor:pointer;font-size:13px;font-weight:600;color:#2e7d4f;text-align:left;min-height:44px;box-sizing:border-box">&#10010; Produkt frei erfassen <span style="opacity:.5;font-weight:400">(ohne Katalog)</span></button>';
    html+='<div id="soc-free-form" style="display:none;margin-top:6px;background:#fbfcfb;border:1px solid #e6e7e4;border-radius:8px;padding:10px"><div style="display:flex;gap:6px;flex-wrap:wrap;align-items:flex-end"><div style="flex:2;min-width:140px"><label style="font-size:10px;font-weight:600;color:#6b7280;display:block">Name *</label><input id="soc-free-name" placeholder="z.B. Kartoffelsalat" style="width:100%;font-size:12px;padding:5px 8px;box-sizing:border-box;border:1px solid #d1d5db;border-radius:6px"></div><div style="width:70px"><label style="font-size:10px;font-weight:600;color:#6b7280;display:block">Preis &euro;</label><input id="soc-free-preis" type="text" inputmode="decimal" placeholder="3,50" style="width:100%;font-size:12px;padding:5px 8px;box-sizing:border-box;border:1px solid #d1d5db;border-radius:6px"></div><div style="flex:1;min-width:100px"><label style="font-size:10px;font-weight:600;color:#6b7280;display:block">Kategorie</label><select id="soc-free-kat" style="width:100%;font-size:12px;padding:5px 8px;box-sizing:border-box;border:1px solid #d1d5db;border-radius:6px"><option value="" disabled selected>\u2014 Kategorie w\u00e4hlen \u2014</option>'+socKatOptionsHtml('')+'<option value="Sonstiges">Sonstiges</option></select></div><div style="width:56px"><label style="font-size:10px;font-weight:600;color:#6b7280;display:block">ab</label><select id="soc-free-ab" style="width:100%;font-size:12px;padding:5px 8px;box-sizing:border-box;border:1px solid #d1d5db;border-radius:6px"><option value="">ab</option><option value="10:00">10:00</option><option value="12:00">12:00</option></select></div></div>';
    html+='<div style="display:flex;gap:6px;align-items:center;margin-top:6px"><label style="cursor:pointer;padding:4px 10px;border-radius:6px;background:#fff;border:1px solid #d1d5db;font-size:11px;color:#374151">&#128247; Bild <input type="file" id="soc-free-bild" accept="image/*" capture="environment" onchange="socialFreeImgPreview()" style="display:none"></label><button onclick="socialFreePaste(this)" title="Bild einfuegen" style="padding:4px 8px;border-radius:6px;background:#fff;border:1px solid #d1d5db;font-size:11px;cursor:pointer;color:#374151">&#128203; Einf\u00fcgen</button><span id="soc-free-img-name" style="font-size:10px;color:#9ca3af;flex:1"></span><button onclick="socialFreeAdd()" style="background:#2e7d4f;color:#fff;padding:5px 14px;font-size:12px;font-weight:600;border:none;border-radius:6px;cursor:pointer">&#10003; Hinzuf\u00fcgen</button></div></div></div>';
    ...
    if(_socialKatalog.length){ html+='<div style="margin-bottom:8px"><input id="soc-pick-search" type="text" placeholder="&#128269; Produkt suchen..." oninput="socialPickFilter()" style="width:100%;padding:8px 12px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;outline:none;box-sizing:border-box"></div>';
      html+='<div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px"><button class="soc-cat-chip soc-cat-active" data-cat="" onclick="socialPickCat(this)" style="padding:4px 10px;border-radius:16px;border:1px solid #d1d5db;background:#1f2937;color:#fff;font-size:11px;font-weight:600;cursor:pointer">Alle</button>';
      ...
      _socialKatalog.forEach(function(p){ var pid=esc(p.id); var priceStr=''; if(p.preis){var cp=parseFloat(p.preis);priceStr=(cp&&isFinite(cp)?cp.toFixed(2).replace('.',','):esc(p.preis))+'\u20AC';}
        ...
        if(p.bild_url) html+='<img id="soc-pick-img-'+pid+'" src="'+esc(p.thumb_url||p.bild_url)+'" data-full="'+esc(p.bild_url)+'" loading="lazy" decoding="async" ondblclick="dlImagePopup(this.getAttribute(\'data-full\')||this.src,\''+esc(p.name).replace(/'/g,"\\'")+'\')" style="width:40px;height:40px;object-fit:cover;border-radius:6px;border:1px solid #e5e7eb;display:block;cursor:zoom-in" onerror="this.style.display=\'none\'">';
        ...
        html+='<input type="text" inputmode="decimal" class="soc-pick-preis" data-id="'+pid+'" value="'+priceVal+'" placeholder="\u20AC" style="width:60px;font-size:11px;padding:2px 4px;border:1px solid #d1d5db;border-radius:4px;color:#2e7d32;font-weight:600;text-align:right">';
  ...
  function socialRenderFreeItems(){ if(!_socFreeItems.length) return ''; var h=''; _socFreeItems.forEach(function(fi){ h+='<div data-id="'+esc(fi.id)+'" style="display:flex;align-items:center;gap:6px;padding:5px 8px;margin-bottom:3px;background:#eef6f0;border:1px solid #cfe6d6;border-radius:8px">'; if(fi.bild_data) h+='<img src="'+fi.bild_data+'" style="width:28px;height:28px;object-fit:cover;border-radius:4px;flex-shrink:0">'; h+='<span style="font-weight:600;font-size:12px;flex:1">'+esc(fi.name)+(fi.ab_uhr?' <span style="font-size:10px;color:#9ca3af;font-style:italic">ab '+esc(fi.ab_uhr)+'</span>':'')+'</span>'; if(fi.preis) h+='<span style="font-size:11px;color:#2e7d32;font-weight:600">'+esc(fi.preis)+'\u20AC</span>'; h+='<span style="font-size:10px;color:#6b7280;background:#e0e7ff;padding:1px 6px;border-radius:8px">'+esc(fi.kategorie)+'</span>'; h+='<button onclick="socialFreeRemove(\''+esc(fi.id)+'\')" style="border:none;background:none;color:#dc2626;cursor:pointer;font-size:14px;padding:0 4px">&#10005;</button></div>'; }); return h; }
  ...
  window.socialFreeAdd=function(){var name=(document.getElementById('soc-free-name').value||'').trim();var preis=(document.getElementById('soc-free-preis').value||'').trim();var kat=document.getElementById('soc-free-kat').value;if(!name){socialStatus('soc-post-status','Bitte Name eingeben',false);return;}if(!kat){socialStatus('soc-post-status','Bitte Kategorie w\u00e4hlen',false);return;}var _max=window.SOC_MAX_ITEMS||3;if(socialGatherSelected().length>=_max){socialStatus('soc-post-status','Maximal '+_max+' Produkte pro Post \u2013 bitte zuerst eines abw\u00e4hlen.',false);return;}var bildInp=document.getElementById('soc-free-bild');var preisNum=parseFloat(preis.replace(',','.'));var abUhr=(document.getElementById('soc-free-ab')||{}).value||'';var fi={id:'free-'+(++_socFreeCounter),name:name,preis:preisNum&&isFinite(preisNum)?preisNum.toFixed(2):'',kategorie:kat,bild_data:'',ab_uhr:abUhr};function finish(){_socFreeItems.push(fi);var list=document.getElementById('soc-free-list');if(list) list.innerHTML=socialRenderFreeItems();document.getElementById('soc-free-name').value='';document.getElementById('soc-free-preis').value='';var katSel2=document.getElementById('soc-free-kat');if(katSel2) katSel2.value='';var abSel2=document.getElementById('soc-free-ab');if(abSel2) abSel2.value='';if(bildInp) bildInp.value='';var lbl=document.getElementById('soc-free-img-name');if(lbl) lbl.textContent='';window._socFreePastedData='';socialPickUpdate();}if(window._socFreePastedData){fi.bild_data=window._socFreePastedData;finish();}else if(bildInp&&bildInp.files&&bildInp.files[0]){var reader=new FileReader();reader.onload=function(e){fi.bild_data=e.target.result;finish();};reader.readAsDataURL(bildInp.files[0]);}else{finish();}};
