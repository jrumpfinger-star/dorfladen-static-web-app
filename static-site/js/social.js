...
(function(){
  ...
  window.socialFreeAdd=function(){var name=(document.getElementById('soc-free-name').value||'').trim();var preis=(document.getElementById('soc-free-preis').value||'').trim();var kat=document.getElementById('soc-free-kat').value;if(!name){socialStatus('soc-post-status','Bitte Name eingeben',false);return;}var _max=window.SOC_MAX_ITEMS||3;if(socialGatherSelected().length>=_max){socialStatus('soc-post-status','Maximal '+_max+' Produkte pro Post \u2013 bitte zuerst eines abw\u00e4hlen.',false);return;}var bildInp=document.getElementById('soc-free-bild');var preisNum=parseFloat(preis.replace(',','.'));var abUhr=(document.getElementById('soc-free-ab')||{}).value||'';var fi={id:'free-'+(++_socFreeCounter),name:name,preis:preisNum&&isFinite(preisNum)?preisNum.toFixed(2):'',kategorie:kat,bild_data:'',ab_uhr:abUhr};function finish(){_socFreeItems.push(fi);var list=document.getElementById('soc-free-list');if(list) list.innerHTML=socialRenderFreeItems();document.getElementById('soc-free-name').value='';document.getElementById('soc-free-preis').value='';var abSel2=document.getElementById('soc-free-ab');if(abSel2) abSel2.value='';if(bildInp) bildInp.value='';var lbl=document.getElementById('soc-free-img-name');if(lbl) lbl.textContent='';window._socFreePastedData='';socialPickUpdate();}if(window._socFreePastedData){fi.bild_data=window._socFreePastedData;finish();}else if(bildInp&&bildInp.files&&bildInp.files[0]){var reader=new FileReader();reader.onload=function(e){fi.bild_data=e.target.result;finish();};reader.readAsDataURL(bildInp.files[0]);}else{finish();}};
