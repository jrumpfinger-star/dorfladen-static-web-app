...
+'@media(hover:hover){.el-zone:hover .rz-handle{display:block}}'
  ...
  function socialShareFiles(files,msg,hasMt){
    ...
    // Nur das erste Bild teilen (WhatsApp-kompatibel); canShare gegen genau diese
    // Auswahl pruefen, damit mehrere/grosse Bilder das native Teilen nicht blockieren.
    var shareSet=files.length>1?[files[0]]:files;
    var canShareFiles=false;
    if(navigator.share&&navigator.canShare){
      try{canShareFiles=navigator.canShare({files:shareSet});}catch(e){}
    }
    console.log('[Social] canShareFiles='+canShareFiles);
    // Best case: share with files
    if(canShareFiles){
      // WICHTIG: KEIN navigator.clipboard.writeText() VOR navigator.share()!
      // Auf Android verbraucht der Clipboard-Schreibzugriff die transiente Nutzer-
      // aktivierung, wodurch navigator.share() mit NotAllowedError fehlschlaegt und
      // die App in den Download-Fallback faellt. Text erst NACH dem Teilen kopieren.
      var shareData={files:shareSet};
