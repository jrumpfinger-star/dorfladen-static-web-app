...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    record_id = req.params.get("id", "").strip()
    if not record_id:
        return func.HttpResponse(
            json.dumps({"success": False, "error": "id parameter is required"}, ensure_ascii=False),
    ...
    for env_name, base_url in _env_candidates():
        ...
        delete_url = f"{base_url}/api/data/v9.2/{entity_set}({record_id})"
        r = requests.delete(delete_url, headers=headers, timeout=30)
        if r.status_code in (200, 204):
            return func.HttpResponse(
                json.dumps({"success": True, "action": "deleted", "id": record_id}, ensure_ascii=False),
                status_code=200, mimetype="application/json", headers=get_cors_headers()
            )
        return func.HttpResponse(
            json.dumps({"success": False, "error": f"DELETE failed: {r.status_code} {r.text[:300]}"}, ensure_ascii=False),
